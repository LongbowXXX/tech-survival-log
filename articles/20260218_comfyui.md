...
> ⚠️ セキュリティ注意  
> `curl ... | sh` 形式のインストールは、ダウンロードしたスクリプトを即座に実行するため、悪意のあるコードが混入していた場合のリスクがあります。  
> 本番環境や慎重を期す場合は、以下のいずれかの方法を推奨します：
>
> - **スクリプト内容を事前確認：** `curl -LsSf https://astral.sh/uv/install.sh | less` でスクリプトの中身を確認してから実行
> - **Homebrew経由でインストール：** `brew install uv`（Homebrewを使用している場合）
> - **公式バイナリを手動ダウンロード：** [GitHubのReleases](https://github.com/astral-sh/uv/releases)から直接ダウンロード
>
> 今回は個人の実験環境であり、公式の信頼できるソースからのインストールであるため、このまま実行しています。
...
> ⚠️ セキュリティ注意  
> `--listen` を付けると、ComfyUI が LAN 上の他の端末からもアクセス可能になります。ComfyUI 側には強力な認証機構がないため、**信頼できるネットワーク内でのみ** 利用してください。  
> 公共 Wi‑Fi やオフィスなど第三者が同一ネットワーク上にいる可能性がある環境では、以下のいずれかを検討してください：
>
> - macOS ファイアウォールで 8188 番ポートへのアクセス元を限定する
> - 直接公開せず、SSH トンネル越しに `localhost:8188` へ接続する
> - 認証付きのリバースプロキシ（例：nginx + Basic 認証など）を挟んで公開する
>
> LAN 共有が不要な場合は、`--listen` を付けずに起動すると `localhost` のみで待ち受けるため、より安全です。
> ブラウザで `http://localhost:8188/` にアクセスし、UIが表示されれば成功です。
