...
今回は、しばらく前に生成AIの実験用に購入した **Mac Mini M4 Pro (メモリ64GB)** を活用し、ノードベースの画像や音声などの生成GUI [ComfyUI](https://github.com/Comfy-Org/ComfyUI) の環境を構築した記録を共有します。
...
Pythonの環境管理には、`uv` を採用します。Rust製で速い。早いのは良いことです。
